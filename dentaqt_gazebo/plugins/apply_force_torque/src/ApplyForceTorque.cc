...
#include <dentaqt_gazebo/ApplyForceTorque.hh>
#include <dentaqt_gazebo/WrenchVisualizer.hh>
...
namespace gz
{
    namespace sim
    {
        ...
        public:
            void PublishWrench(bool _applyForce, bool _applyTorque);
        ...
        public:
            rendering::RayQueryPtr ray{nullptr};
        ...
        public:
            bool sendBlockOrbit{false};

            /// \brief Where the mouse left off
        public:
            math::Vector2i mousePressPos = math::Vector2i::Zero;
...
void ApplyForceTorque::LoadConfig(const tinyxml2::XMLElement * /*_pluginElem*/)
{
    ...
    if (!worldNames.empty())
    {
        ...
        if (topic == "")
        {
            ignerr << "Unable to create publisher" << std::endl;
            return;
        }
        this->dataPtr->pub =
            this->dataPtr->node.Advertise<msgs::EntityWrench>(topic);
        igndbg << "Created publisher to " << topic << std::endl;
    }
                              ...
                              EntityComponentManager &_ecm)
{
    ...
    if (!this->dataPtr->systemLoaded)
    {
        ...
        if (!msg)
        {
            igndbg << "Unable to find SystemPluginInfo component for entity "
        ...
        for (const auto &plugin : msg->plugins())
        {
            if (plugin.filename() == filename)
            {
                ...
                igndbg << "ApplyLinkWrench system already loaded" << std::endl;
                break;
            }
        }
        ...
        if (!this->dataPtr->systemLoaded)
        {
            ...
            {
                this->dataPtr->systemLoaded = true;
                igndbg << "ApplyLinkWrench system has been loaded" << std::endl;
            }
            else
            {
...
void ApplyForceTorque::SetForce(QVector3D _force)
{
    ...
    if (this->dataPtr->activeVector == RotationToolVector::FORCE && this->dataPtr->activeAxis == rendering::TransformAxis::TA_NONE)
    {
        this->dataPtr->vectorRot = math::Matrix4d::LookAt(
                                       -this->dataPtr->force, math::Vector3d::Zero)
                                       .Rotation();
    }
...
void ApplyForceTorque::SetTorque(QVector3D _torque)
{
    ...
    if (this->dataPtr->activeVector == RotationToolVector::TORQUE && this->dataPtr->activeAxis == rendering::TransformAxis::TA_NONE)
    {
        this->dataPtr->vectorRot = math::Matrix4d::LookAt(
                                       -this->dataPtr->torque, math::Vector3d::Zero)
                                       .Rotation();
    }
...
void ApplyForceTorquePrivate::PublishWrench(bool _applyForce, bool _applyTorque)
{
    ...
    {
        igndbg << "No link selected" << std::endl;
        return;
    }

    // Force and torque in world coordinates
    math::Vector3d forceToApply = _applyForce ? this->linkWorldPose.Rot().RotateVector(this->force) : math::Vector3d::Zero;
    math::Vector3d torqueToApply = _applyTorque ? this->linkWorldPose.Rot().RotateVector(this->torque) : math::Vector3d::Zero;
    // The ApplyLinkWrench system takes the offset in the link frame
    math::Vector3d offsetToApply = _applyForce ? this->offset + this->inertialPose.Pos() : math::Vector3d::Zero;
...
void ApplyForceTorquePrivate::OnRender()
{
    if (!this->scene)
    {
        ...
        for (unsigned int i = 0; i < this->scene->SensorCount(); ++i)
        {
            ...
            {
                this->camera = cam;
                igndbg << "ApplyForceTorque plugin is using camera ["
...
void ApplyForceTorquePrivate::UpdateVisuals()
{
        ...
        this->selectedEntity.has_value())
    {
        ...
        double scale = applicationPoint.Distance(this->camera->WorldPose().Pos()) / 2.0;
    ...
    {
        math::Vector3d pos;
        if (this->activeVector == RotationToolVector::FORCE && this->force != math::Vector3d::Zero)
        {
            pos =
                inertialWorldPose.Pos() +
                this->linkWorldPose.Rot().RotateVector(this->offset);
        }
        else if (this->activeVector == RotationToolVector::TORQUE && this->torque != math::Vector3d::Zero)
        {
        ...
        double scale = pos.Distance(this->camera->WorldPose().Pos()) / 2.0;
        ...
        math::Vector3d yRotOffset(M_PI * 0.5, -M_PI * 0.5, 0);
        ...
        math::Vector3d circleRotOffset(0, M_PI * 0.5, 0);
...
void ApplyForceTorquePrivate::HandleMouseEvents()
{
    ...
    if (this->mouseEvent.Button() == common::MouseEvent::LEFT)
    {
        ...
        if (this->mouseEvent.Type() == common::MouseEvent::PRESS && this->activeVector != RotationToolVector::NONE)
        {
            ...
            if (visual)
            {
                ...
                if (axis == rendering::TransformAxis::TA_ROTATION_Y || axis == rendering::TransformAxis::TA_ROTATION_Z)
                {
    ...
    if (this->mouseEvent.Type() == common::MouseEvent::MOVE && this->activeAxis != rendering::TransformAxis::TA_NONE)
    {
        this->blockOrbit = true;
        this->sendBlockOrbit = true;
...
IGNITION_ADD_PLUGIN(ApplyForceTorque, gz::gui::Plugin)
