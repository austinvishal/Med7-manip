...
// package headers
#include <dentaqt_ign_ros2_control/dentaqt_ign_system.hpp>
...
struct jointData {
  ...
  dentaqt_ign_ros2_control::DentaqtIgnitionSystemInterface::ControlMethod
...
public:
  ...
  /// \brief ft's topic name.
  std::string topicName{};

  /// \brief handles to the ft from within Gazebo
...
void ForceTorqueData::OnForceTorque(const ignition::msgs::Wrench &_msg) {
  ...
  this->ft_sensor_data_[4] = _msg.torque().x();
  this->ft_sensor_data_[5] = _msg.torque().y();
}
...
class dentaqt_ign_ros2_control::DentaqtIgnitionSystemPrivate {
public:
  DentaqtIgnitionSystemPrivate() = default;
...
bool DentaqtIgnitionSystem::initSim(
  ...
  this->dataPtr = std::make_unique<DentaqtIgnitionSystemPrivate>();
  ...
  for (unsigned int j = 0; j < this->dataPtr->n_dof_; j++) {
    ...
    this->dataPtr->joints_[j].joint_type =
        _ecm.Component<ignition::gazebo::components::JointType>(simjoint)
            ->Data();

    // Create joint position component if one doesn't exist
    if (!_ecm.EntityHasComponentType(
            simjoint, ignition::gazebo::components::JointPosition().TypeId())) {
    ...
    if (!_ecm.EntityHasComponentType(
            simjoint, ignition::gazebo::components::JointVelocity().TypeId())) {
      _ecm.CreateComponent(simjoint,
    ...
    if (!_ecm.EntityHasComponentType(
      ...
      _ecm.CreateComponent(
          simjoint, ignition::gazebo::components::JointTransmittedWrench());
    }

    // Create joint axis component if one doesn't exist
    if (!_ecm.EntityHasComponentType(
            simjoint, ignition::gazebo::components::JointAxis().TypeId())) {
      _ecm.CreateComponent(simjoint, ignition::gazebo::components::JointAxis());
    }
...
void DentaqtIgnitionSystem::registerSensors(
    const hardware_interface::HardwareInfo &hardware_info) {
  ...
  this->dataPtr->ecm->Each<ignition::gazebo::components::Imu,
      ...
      [&](const ignition::gazebo::Entity &_entity,
        ...
        auto sensorTopicComp =
            this->dataPtr->ecm
                ->Component<ignition::gazebo::components::SensorTopic>(_entity);
        if (sensorTopicComp) {
          RCLCPP_INFO_STREAM(this->nh_->get_logger(),
        ...
        static const std::map<std::string, std::size_t> interface_name_map = {
...
CallbackReturn DentaqtIgnitionSystem::on_init(
...
CallbackReturn DentaqtIgnitionSystem::on_configure(
...
DentaqtIgnitionSystem::export_state_interfaces() {
  return std::move(this->dataPtr->state_interfaces_);
}

std::vector<hardware_interface::CommandInterface>
DentaqtIgnitionSystem::export_command_interfaces() {
  return std::move(this->dataPtr->command_interfaces_);
}

CallbackReturn DentaqtIgnitionSystem::on_activate(
    const rclcpp_lifecycle::State &previous_state) {
  return CallbackReturn::SUCCESS;
  return hardware_interface::SystemInterface::on_activate(previous_state);
}

CallbackReturn DentaqtIgnitionSystem::on_deactivate(
    const rclcpp_lifecycle::State &previous_state) {
  return CallbackReturn::SUCCESS;
...
DentaqtIgnitionSystem::read(const rclcpp::Time & /*time*/,
  ...
  for (unsigned int i = 0; i < this->dataPtr->joints_.size(); ++i) {
    ...
    //         this->dataPtr->joints_[i].sim_joint);
    // see as a reference to get the joint torque
    // https://github.com/robotology/gz-sim-yarp-plugins/blob/41c9ab292505a75334ba380a3bb1a86670a055f8/plugins/controlboard/src/ControlBoard.cpp#L311-L321
    // https://github.com/ros-controls/gz_ros2_control/pull/186
    const auto *jointTransmittedWrench =
        this->dataPtr->ecm
            ->Component<ignition::gazebo::components::JointTransmittedWrench>(
                this->dataPtr->joints_[i].sim_joint);
    ...
    double jointForce{};
    auto axis = ignition::math::Vector3d(jointAxis->Data().Xyz().X(),
                                         jointAxis->Data().Xyz().Y(),
                                         jointAxis->Data().Xyz().Z());
    if (this->dataPtr->joints_[i].joint_type == sdf::JointType::PRISMATIC) {
      auto wrenchForce =
          ignition::math::Vector3d(jointTransmittedWrench->Data().force().x(),
                                   jointTransmittedWrench->Data().force().y(),
                                   jointTransmittedWrench->Data().force().z());
      jointForce = wrenchForce.Dot(axis);
    } else if (this->dataPtr->joints_[i].joint_type ==
                   sdf::JointType::REVOLUTE ||
               this->dataPtr->joints_[i].joint_type ==
                   sdf::JointType::CONTINUOUS) {
      auto wrenchTorque =
          ignition::math::Vector3d(jointTransmittedWrench->Data().torque().x(),
                                   jointTransmittedWrench->Data().torque().y(),
                                   jointTransmittedWrench->Data().torque().z());
      jointForce = wrenchTorque.Dot(axis);
    }
    ...
    this->dataPtr->joints_[i].joint_effort = jointForce;
  }
  ...
  for (unsigned int i = 0; i < this->dataPtr->imus_.size(); ++i) {
    if (this->dataPtr->imus_[i]->topicName.empty()) {
      auto sensorTopicComp =
          this->dataPtr->ecm
              ->Component<ignition::gazebo::components::SensorTopic>(
  ...
  for (unsigned int i = 0; i < this->dataPtr->ft_sensors_.size(); ++i) {
    if (this->dataPtr->ft_sensors_[i]->topicName.empty()) {
      ...
      if (sensorTopicComp) {
        ...
        RCLCPP_INFO_STREAM(this->nh_->get_logger(),
                           "FT " << this->dataPtr->ft_sensors_[i]->name
                                 << " has a topic name: "
                                 << sensorTopicComp->Data());
...
hardware_interface::return_type
DentaqtIgnitionSystem::perform_command_mode_switch(
...
hardware_interface::return_type
DentaqtIgnitionSystem::write(const rclcpp::Time & /*time*/,
                             const rclcpp::Duration & /*period*/) {
  for (unsigned int i = 0; i < this->dataPtr->joints_.size(); ++i) {
    if (this->dataPtr->joints_[i].joint_control_method & VELOCITY) {
      if (!this->dataPtr->ecm
               ->Component<ignition::gazebo::components::JointVelocityCmd>(
                   this->dataPtr->joints_[i].sim_joint)) {
        this->dataPtr->ecm->CreateComponent(
            this->dataPtr->joints_[i].sim_joint,
      ...
      if (!this->dataPtr->ecm
               ->Component<ignition::gazebo::components::JointForceCmd>(
                   this->dataPtr->joints_[i].sim_joint)) {
        this->dataPtr->ecm->CreateComponent(
            this->dataPtr->joints_[i].sim_joint,
    ...
    for (const auto &mimic_interface : mimic_joint.interfaces_to_mimic) {
      ...
      if (mimic_interface == "effort") {
        ...
        if (!this->dataPtr->ecm->Component<
          ...
          const auto jointEffortCmd = this->dataPtr->ecm->Component<
              ignition::gazebo::components::JointForceCmd>(
              this->dataPtr->joints_[mimic_joint.joint_index].sim_joint);
          *jointEffortCmd = ignition::gazebo::components::JointForceCmd(
              {mimic_joint.multiplier *
...
PLUGINLIB_EXPORT_CLASS(dentaqt_ign_ros2_control::DentaqtIgnitionSystem,
                       dentaqt_ign_ros2_control::DentaqtIgnitionSystemInterface)
